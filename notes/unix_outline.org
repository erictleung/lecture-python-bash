...
Open a terminal window (use PuTTY if you are a Windows user) so you can connect
to the Linux server and try out the commands demonstrated in this lecture.
...
#+BEGIN_SRC sh
  # List out files in your working directory
  ls
#+END_SRC
...
#+BEGIN_SRC sh
  # "Rename" file by copying to new name
...
#+BEGIN_SRC sh
  # Create a new empty file and explore file permissions
  touch restricted_file.txt
  ls -l  # List of files with extra information
#+END_SRC
...
There are two ways to manipulate a file/directory's permissions:
...
#+BEGIN_SRC sh
  # Change files permissions with symbols
...
#+BEGIN_SRC sh
  # Update the terminal's knowledge of the $PATH variable
  export PATH
#+END_SRC
...
#+BEGIN_SRC sh
  # Take file and input it to a command
  head < list_of_files.txt
#+END_SRC
...
#+BEGIN_SRC sh
  ...
  ls -l /home/courses/BMI565/students | awk '{ print $6,$9 }'
#+END_SRC

#+BEGIN_SRC sh
  # Do some manipulation of entries
  ls -l /home/courses/BMI565/students | awk '{ print $7 * 2 }'
#+END_SRC
...
#+BEGIN_SRC sh
  ...
  ls -l /home/courses/BMI565/students | awk '
    ...
    $8 < 15 { print $NF, "was edited earlier in that month" }
...
Here is a basic Makefile (named as ~Makefile~ in your directory) that will
count the words of a file and save the results into the file ~word_count.txt~.
...
While the ~Makefile~ is in your working directory, you can run ~make~ in your
terminal to run the rule.
...
*Hint*: Use ~wget/curl~ to download the file and ~awk~ to subset data.
...
*Hint*: Use ~grep~ to count number of occurrence. You can use the ~-o~ flag for
~grep~ to extract matches and ~wc~ to count.
